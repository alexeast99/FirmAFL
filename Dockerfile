...
RUN apt update
RUN apt upgrade -y
RUN apt install -y tzdata git build-essential make vim qemu python2 libssl-dev \
                   libffi-dev python3-dev pkg-config sudo libglib2.0-dev flex \
                   bison autoconf libtool

USER alexeast
WORKDIR /home/alexeast/workspace
RUN git clone https://github.com/alexeast99/FirmAFL --branch dev

WORKDIR /home/alexeast/workspace/FirmAFL
